...
1.  **[Sandbox Proxy (`sandbox.html`)](https://github.com/google/A2UI/blob/main/samples/client/shared/mcp_apps_inner_iframe/sandbox.html)**：從同源提供的中間 `iframe`。它在維持結構化 JSON-RPC 通道的同時，將原始 DOM 注入與主應用隔離。
    - 權限：不要在 host 模板中對它啟用 sandbox（例如 [`mcp-app.ts`](https://github.com/google/A2UI/blob/main/samples/client/angular/projects/mcp_calculator/src/a2ui-catalog/mcp-app.ts) 或 [`mcp-apps-component.ts`](https://github.com/google/A2UI/blob/main/samples/client/lit/custom-components-example/ui/custom-components/mcp-apps-component.ts)）。
...
MCP server 使用 SSE transport 啟動在 `http://localhost:8000`。
...
> ⚠️ **必須使用 `--include=dev`**：Angular CLI（`@angular/cli`）是 dev dependency。如果不使用 `--include=dev`，`ng serve` 不可用。
>
> ⚠️ **`build:renderer` 和 `build:sandbox` 都是必需的**：`build:renderer` 會編譯 Angular app 依賴的 A2UI renderer package。`build:sandbox` 會把 sandbox proxy 打包進 Angular 專案的 public assets。缺少任一步，app 都無法工作。
...
| Python version error on `contact_lookup` agent | 安裝 Python 3.13+（該範例的 `pyproject.toml` 要求）                                             |
| `npm run build:renderer` fails                 | 確保已先在 `samples/client/lit/` 中執行 `npm install`                                           |
| Angular client shows blank page                | 確保在 `npm start` 前執行了 `npm run build:sandbox`                                             |
| MCP app iframe doesn't load                    | 檢查 MCP server（端口 8000）和 proxy agent（端口 10006）是否都在執行                            |
| `ng serve` not found                           | 執行 `npm install --include=dev` 安裝包含 `@angular/cli` 在內的 dev dependencies                 |
