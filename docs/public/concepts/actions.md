...
Renderer 會在發給 agent 的訊息 **metadata** 中包含 `a2uiClientCapabilities` 物件（例如 A2A `Message` 的 `metadata` 欄位）。
...
為了在多 agent 架構中處理路由，orchestrator 必須維護每個 `surfaceId` 到其所屬 sub-agent 的對映。官方 Python SDK 提供了 [`A2uiSubagentMap`](../../../agent_sdks/python/a2ui_agent/src/a2ui/adk/orchestration/a2ui_subagent_map.py) 工具程式，可以安全地管理這份對映。

如需完整、可運作的範例，請參閱 [orchestrator agent executor](../../../samples/community/agent/adk/orchestrator/orchestrator_agent_executor.py) 範例。

#### 1. 從 Server Event 對映所有權

當 sub-agent 發出 A2UI `createSurface` 或 `deleteSurface` 時，orchestrator 會攔截該訊息，並使用 `update_from_server_event` 把所有權記錄到 session state 中。
...
當 client renderer 把 A2UI `action` 或 `error` 訊息送回時，orchestrator 會使用 `get_subagent_name_for_client_event` 在 session state 中查找 `surfaceId`，並將請求路由給正確的 sub-agent。
...
該模式確保雙向通訊循環仍能為每個功能區域維持完整且有狀態。

### 透過 Metadata Stripping 防止資料泄漏

在多 agent 環境中，`a2uiClientDataModel` 物件可能包含由不同 sub-agent 擁有的多個 surface 狀態。為防止敏感資料泄漏，orchestrator 必須 **strip** data model metadata，只保留目標 sub-agent 所擁有的 surface。

你可以在 outbound interceptor 中使用 `strip_unowned_surfaces_from_data_model` 方法，直接就地修改 data model 字典。
...
透過剥離 metadata，orchestrator 可以確保每個 sub-agent 都只會收到自己有權查看的 data model 部分。

WARNING: **安全風險 - 狀態抓取**：如果 orchestrator 未剥離 `a2uiClientDataModel`，惡意或遭入侵的 sub-agent 就可能讀取其他 active surface 的狀態。例如，如果 orchestrator 泄漏了整個多 surface data model，一個天氣 sub-agent 就可能從銀行 surface 抓取敏感資料。剥離是多 agent 系統的強制安全要求。
