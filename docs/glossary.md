...
參見 [basic catalog](../specification/v0_10/catalogs/basic/catalog.json)。
...
- **Catalog Schema**：
    ...
    例如，參見 [basic catalog schema](../specification/v0_10/catalogs/basic/catalog.json)。
...
參見 [資料流指南](concepts/data-flow.md)。
...
參見 [資料繫結指南](concepts/data-binding.md)。

### Data reference

在元件定義中，對某個資料元素的引用；該引用可以透過 data model 中的路徑解析，也可以按值解析。

參見 [basic catalog 中的範例](../specification/v0_10/catalogs/basic/catalog.json#L18)。
...
參見 [actions 詳細指南](concepts/actions.md)。
