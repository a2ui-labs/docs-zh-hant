...
[查看 Rizzcharts 目錄範例](../../../samples/community/agent/adk/rizzcharts/catalog_schemas/0.9/rizzcharts_catalog_definition.json)
...
### 目錄連結（Catalog Linking）

A2UI 目錄必須是獨立的，不可引用外部檔案，這樣才能簡化 LLM 推理與相依管理。

雖然最終目錄必須是獨立的，但在本地開發時，你仍然可以用 JSON Schema `$ref` 指向外部文件來模組化撰寫目錄。

為了自動化打包並註冊這些外部檔案參照，這個目錄註冊流程稱為 **「Linking」**，並整合成單一、跨平台的 Node.js 腳本（**`register-catalogs.js`**）。

這個連結腳本已原生封裝在 **Xcode Build Phases**（用於 iOS/macOS 客戶端建置）與 **Gradle tasks**（用於 Android 客戶端建置）之中，能在應用程式建置階段無縫地編譯、彙整並連結靜態與動態 schema。
...
**請務必透過你所在平台的 Xcode Build Phase 或 Gradle task（執行 `register-catalogs.js`）在編譯期連結並解析外部參照。**
