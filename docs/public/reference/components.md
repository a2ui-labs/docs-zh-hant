...
!!! abstract "Schema 檔案"
    ...
    === "v0.8"
        ...
        [:material-code-json: 標準 Catalog 定義（JSON Schema）](../../../specification/v0_8/json/standard_catalog_definition.json)

    === "v0.9"

        [:material-code-json: 基本 Catalog 定義（JSON Schema）](../../../specification/v0_9/catalogs/basic/catalog.json)
...
> [!NOTE]
> **套件管理工具的使用：** 在 A2UI 倉庫內執行內建的示例應用程式時，需要使用 Yarn（`yarn start gallery`），因為這是透過 Corepack workspaces 設定的。若是在此倉庫之外的一般日常使用與獨立專案中，可以自由選擇你偏好的套件管理工具（例如 npm、pnpm）。
