...
=== "v0.9 and later"
    ...
    v0.9 及後續版本採用更扁平的元件格式：`"component": "Text"` 取代巢狀的 `{"Text": {...}}`，而 children 也改成單純陣列，不再使用 `{"explicitList": [...]}`。
...
=== "v0.9 and later"
    ...
    1. LLM 透過 `createSurface` 建立 surface（並指定 catalog；在 v1.0 中，也可以一併包含初始的 data model 與元件）
