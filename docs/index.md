...
| 版本 | 狀態 | 說明 |
|---------|--------|-------------|
| **[v0.8](specification/v0.8-a2ui.md)** | **穩定版** | 目前可用於生產的版本。包含 surface、component、data binding 與 adjacency list 模型。 |
| **[v0.9](specification/v0.9-a2ui.md)** | **草案** | 新增 `createSurface`、客戶端函式、自訂 catalog 與擴充規範。[查看演進指南 →](specification/v0.9-evolution-guide.md) |

A2UI 採用 Apache 2.0 授權，
由 Google 發起，CopilotKit 與開源社群共同參與貢獻，
並持續在 [GitHub](https://github.com/google/A2UI) 上活躍開發中。
...
在這個倉庫中，你會找到
[A2UI 規範](specification/v0.8-a2ui.md)（v0.8 穩定版、v0.9 草案），
...
- :material-shield-check: **從設計上確保安全**
    ...
    它是宣告式資料格式，而不是可執行程式碼。智慧體只能使用你在 catalog 中預先核准的元件，從根源避免 UI 注入攻擊。
...
- :material-file-document:{ .lg .middle } **協議規範**
    ...
    深入閱讀完整技術規範：[v0.8（穩定版）](specification/v0.8-a2ui.md) · [v0.9（草案）](specification/v0.9-a2ui.md)

    [:octicons-arrow-right-24: 閱讀 v0.8 規範](specification/v0.8-a2ui.md)
